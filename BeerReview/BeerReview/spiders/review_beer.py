...
import scrapy
import re
from BeerReview.items import ReviewItem
	...
	review_container = response.xpath('//*[@id="container"]/div[2]/div[2]/div[7]').extract()
        ...
        for rev in range(1, num_reviews+1):
            try:
                ...
                user_rating = None
            items["user_rating"] = user_rating

            try:
                aroma = \
                    Selector(text=review_container[0]).xpath('//div/div/div['+str(2*rev-1)+']/strong/big[1]/text()').extract()[0]
            except:
                aroma = None
            items["aroma"] = aroma
            ...
            try:
                appearance = \
                    Selector(text=review_container[0]).xpath('//div/div/div['+str(2*rev-1)+']/strong/big[2]/text()').extract()[0]
            except:
                appearance = None
            ...
            try:
                taste = \
                    Selector(text=review_container[0]).xpath('//div/div/div['+str(2*rev-1)+']/strong/big[3]/text()').extract()[0]
            except:
                taste = None
            items["taste"] = taste
            ...
            try:
                palate = \
                    Selector(text=review_container[0]).xpath('//div/div/div['+str(2*rev-1)+']/strong/big[4]/text()').extract()[0]
            except:
                palate = None
            items["palate"] = palate
            ...
            try:
                overall = \
                    Selector(text=review_container[0]).xpath('//div/div/div['+str(2*rev-1)+']/strong/big[5]/text()').extract()[0]
            except:
                overall = None
